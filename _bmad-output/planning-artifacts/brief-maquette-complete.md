...
|---|---|---|---|
| 00 | Navigation *(pas une page)* | en-tête au repos / réduit au scroll ; **menu « À propos » ouvert au survol** (les trois ancres : Qui suis-je ? · Ma méthode · Cible) ; **menu mobile déplié avec « À propos » en accordéon** ; **variante réseaux sociaux A = dans la nav, à gauche du logo** et **variante B = en pied de page seulement** (D-6) ; pied de page **avec le lockup co-brandé Anne + eXp en négatif** (nouveau, charte § 2.4 — à valider avec Anne en séance) ; sélecteur de langue ouvert | § 0 (v4) |
...
- **Logos** : `design-system/assets/` (16 SVG : **6 primaires sans sous-titre** — horizontal, empilé, monogramme, symbole, négatif, mono-brun (charte § 2, 2026-09-22 : le sous-titre « Conseil immobilier — Léman & Chablais » quitte les logos, illisible en réduction) · **4 avec descripteur**, réservées aux supports print ≥ seuil (recto carte, papeterie, couvertures) · **4 lockups co-brandés eXp** — `lockup-exp-horizontal` (défaut), `-negatif` (fonds sombres, **c'est celui du pied de page**), `-sans-ligne`, `-centre`, tous régénérés avec le logo Anne sans sous-titre · les 2 originaux eXp Black/White, intouchables). Règle (charte § 2.4, 2026-09-22) : eXp en pied de page et page légale uniquement, jamais en en-tête, jamais recoloré. En maquette, le secteur se dit en texte (sous-titre du hero, ligne de statut du pied de page), pas dans le logo.
