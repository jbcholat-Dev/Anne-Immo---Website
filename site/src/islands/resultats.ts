  ...
  if (!r || Date.now() - r.at > 24 * 3600 * 1000) {
    ...
    zone.innerHTML = `
      ...
      ${A
        ...
        : `<div class="rs-sequence"><p>${esc((s as any).sequence)}</p><button type="button" class="btn btn-secondaire btn-m" data-sequence ${r.newsletter ? 'hidden' : ''}>${esc((s as any).sequence_cta)}</button><span data-sequence-deja ${r.newsletter ? '' : 'hidden'} class="texte-muet" style="font-size:14px">Séquence de 7 e-mails confirmée.</span></div>`}
